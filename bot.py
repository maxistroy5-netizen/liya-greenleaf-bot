...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_text == "🌿 О компании Greenleaf":
        ...
        await update.message.reply_text(
            "🌿 О КОМПАНИИ GREENLEAF\n\n"
            "Greenleaf — международная компания, которая объединяет производство товаров повседневного спроса, собственные бренды и партнёрскую бизнес-систему. Сегодня компания представлена более чем в 42 странах мира.\n\n"
            "🏭 Центр развития компании связан с Сучжоу, Китай. В корпоративных материалах представлены собственные производственные базы, индустриальный парк и три центра исследований и разработок. Это позволяет компании развивать ассортимент и создавать продукты в разных категориях для ежедневного использования.\n\n"
            "🛍 Ассортимент Greenleaf охватывает тысячи товарных позиций — от ухода за лицом и телом до товаров для дома, здоровья, детей, текстиля, аксессуаров и посуды. В экосистеме компании представлены собственные бренды, среди которых Greenleaf, SEALUXE, Pink Pie, iLiFE, CARICH, Kardli и Nilrich.\n\n"
            "🌍 Международное развитие — важная часть Greenleaf: компания расширяет географию, развивает партнёрские структуры и создаёт возможности для людей из разных стран знакомиться с продукцией, обучаться и строить международные команды.\n\n"
            "Хочешь узнать, что именно выпускает компания? Нажми «💚 Продукция и направления». А если интересна бизнес-сторона — «🌍 Возможности Greenleaf».",
            reply_markup=NEWCOMER_COMPANY_KEYBOARD,
        )
        return
    ...
    if user_text == "💚 Продукция и направления":
        ...
        await update.message.reply_text(
            "💚 ПРОДУКЦИЯ И НАПРАВЛЕНИЯ\n\n"
            "Greenleaf — это большой ассортимент товаров повседневного спроса. По структуре нашего прайса продукция представлена в следующих направлениях:\n\n"
            "🌿 уход за кожей лица\n"
            "🧴 уход за кожей тела\n"
            "🦷 уход за полостью рта\n"
            "💄 декоративная косметика и парфюм\n"
            "💇‍♀️ уход за волосами\n"
            "🧼 личная гигиена\n"
            "🌸 женская гигиена\n"
            "💚 здоровье\n"
            ...
            "🧺 средства для стирки\n"
            "🛏 текстиль\n"
            "👟 обувь и уход за обувью\n"
            "👜 сумки, очки и аксессуары\n"
            "🍽 посуда\n\n"
            "Такой ассортимент позволяет знакомиться с Greenleaf не через один отдельный продукт, а через целую систему товаров, которыми люди пользуются каждый день. Можно выбрать интересующее направление и дальше уже подробно разбирать конкретные продукты.\n\n"
            "Следующий раздел — «🌍 Возможности Greenleaf»: там посмотрим, как вокруг продукции строятся обучение, партнёрство и международное развитие.",
            reply_markup=NEWCOMER_COMPANY_KEYBOARD,
        )
        return
    ...
    if user_text == "🌍 Возможности Greenleaf":
        ...
        await update.message.reply_text(
            "🌍 ВОЗМОЖНОСТИ GREENLEAF\n\n"
            "Greenleaf — это не только продукция. Это международная экосистема, в которой человек может выбрать свой формат участия и постепенно развиваться.\n\n"
            "🛍 КЛИЕНТ\nМожно пользоваться продукцией, изучать ассортимент и выбирать товары для себя и семьи.\n\n"
            "🤝 ПАРТНЁР\nМожно изучить партнёрскую систему и маркетинг-план, научиться рассказывать о продукции и возможностях компании и постепенно формировать свою структуру.\n\n"
            "🎓 ОБУЧЕНИЕ И РАЗВИТИЕ\nВ нашей структуре партнёры получают обучение по продукту, маркетинг-плану, коммуникации, работе с командой, лидерству и практическим инструментам. Задача — чтобы человек не оставался один, а понимал, что делать шаг за шагом.\n\n"
            "🌍 МЕЖДУНАРОДНЫЙ БИЗНЕС\nGreenleaf представлена более чем в 42 странах мира. Партнёрская модель позволяет развивать команду не только в своём городе или регионе, но и выстраивать международные связи и структуру в разных странах.\n\n"
            "🚀 РОСТ\nПуть можно начинать постепенно: познакомиться с продуктом, освоить систему, получить первые навыки, научиться работать с людьми и затем развивать собственную команду.\n\n"
            "Если хочешь перейти к тому, как устроены PV, статусы и бонусы, вернись в главное меню и нажми «📊 Маркетинг-план».",
            reply_markup=NEWCOMER_COMPANY_KEYBOARD,
        )
        return
