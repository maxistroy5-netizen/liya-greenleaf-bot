...
def _whatsapp_url(value):
    ...
    if value.startswith(("http://", "https://")): return value
    d = _digits(value)
    return "https://wa.me/" + d if d else ""
...
def _short_social(value, kind):
    ...
    try:
        p = urlparse(value)
        path = p.path.strip("/")
        if kind == "instagram" and path:
            return "@" + path.split("/")[0]
        if kind == "telegram" and path:
            return "@" + path.split("/")[0]
        if kind == "max" and path:
            return "@" + path.split("/")[0]
...
def _draw_brand_mark(c, cx, cy, r):
    ...
    for dx, dy in [(0, .36*r), (-.35*r, .08*r), (.35*r, .08*r), (-.27*r, -.30*r), (.27*r, -.30*r)]:
        c.circle(cx + dx, cy + dy, r * .22, fill=1, stroke=0)
    c.setFillColor(GREEN)
    c.setFont(FONT_BOLD, max(7, r*.25))
    c.drawCentredString(cx, cy - r*.05, "GREENLEAF")
    c.setFont(FONT, max(6, r*.18))
    c.drawCentredString(cx, cy - r*.27, "Leaders | Москва")


def _draw_photo(c, photo_path):
    c.setStrokeColor(GREEN)
    c.setLineWidth(5)
    c.circle(125, 1110, 78, fill=0, stroke=1)
    if photo_path and os.path.exists(photo_path):
        try:
            img = ImageReader(photo_path)
            iw, ih = img.getSize(); box = 144
            scale = max(box / iw, box / ih); dw, dh = iw * scale, ih * scale
            c.saveState(); p = c.beginPath(); p.circle(125, 1110, 70)
            c.clipPath(p, stroke=0, fill=0)
            c.drawImage(img, 125-dw/2, 1110-dh/2, dw, dh, mask="auto")
            c.restoreState(); return
        except Exception: pass
    c.setFillColor(colors.white); c.circle(125, 1110, 70, fill=1, stroke=0)


def generate_business_card(data, photo_path=None, output_path=None):
    if output_path is None:
        fd, output_path = tempfile.mkstemp(prefix="greenleaf_card_", suffix=".pdf"); os.close(fd)
    c = canvas.Canvas(output_path, pagesize=(W, H)); c.setTitle("GREENLEAF electronic business card")
    c.setFillColor(LIGHT); c.rect(0, 0, W, H, fill=1, stroke=0)
    c.setFillColor(colors.HexColor("#E6F0DE")); c.circle(640, 1140, 270, fill=1, stroke=0)
    c.setFillColor(colors.HexColor("#DCEAD2")); c.circle(90, 950, 210, fill=1, stroke=0)
    ...
    c.setFillColor(DARK); _fit(c, name, 48, 895, 390, 39, FONT_BOLD, 20)
    c.setFillColor(GREEN); c.setFont(FONT_BOLD, 20)
    c.drawString(48, 850, "ПАРТНЁР КОРПОРАЦИИ"); c.drawString(48, 820, "GREENLEAF")
    ...
    c.setFillColor(colors.white); c.roundRect(462, 820, 220, 300, 18, fill=1, stroke=0)
    c.drawImage(ImageReader(bio), 482, 890, 180, 180)
    c.setFillColor(GREEN); c.setFont(FONT_BOLD, 14); c.drawCentredString(572, 862, "GREENLEAF CLUB.RU")
    c.setFont(FONT, 10); c.drawCentredString(572, 842, "СКАНИРУЙ QR-КОД")
    ...
    c.setFont(FONT,11); c.drawCentredString(W/2,265,"GREENLEAF CLUB.RU — ВСЁ В ОДНОМ МЕСТЕ")
    c.linkURL(ECOSYSTEM_URL,(190,245,530,285),relative=0)

    c.setFillColor(colors.white); c.setFont(FONT_BOLD,18); c.drawString(48,125,"GREEN FUTURE TOGETHER")
    c.setFont(FONT,11); c.drawString(48,96,BRAND_NAME)
    c.drawRightString(672,96,"МЕЖДУНАРОДНЫЙ БИЗНЕС"); c.drawRightString(672,78,"С РЕАЛЬНЫМИ ВОЗМОЖНОСТЯМИ")
